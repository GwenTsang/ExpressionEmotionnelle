...
def main():
    ...
    # Deduplicate emotion labels while preserving order
    unique_emotions = []
    for emo in EMOTION_LABELS:
        if emo not in unique_emotions:
            unique_emotions.append(emo)
            
    # Check that the columns exist in the DataFrame
    missing_cols = [col for col in unique_emotions if col not in df.columns]
    if missing_cols:
        print(f"Warning: Les colonnes d'émotions suivantes sont absentes : {missing_cols}")
    ...
    # Convert emotions to binary integers
    ...
    for cat in hate_categories:
        ...
        mean_row = {"HATE": cat}
        for emo in unique_emotions:
            mean_row[emo] = cat_df[emo].mean()
        mean_matrix.append(mean_row)
        
    df_means = pd.DataFrame(mean_matrix).set_index("HATE")
    means_csv_path = os.path.join(results_dir, "distribution_emotions_par_hate.csv")
    df_means.to_csv(means_csv_path, float_format='%.4f')
    
    print("=" * 100)
    print(" 1. DISTRIBUTION DES EMOTIONS PAR CATEGORIE DE HATE (Proportion de présence)")
    print("=" * 100)
    ...
    for cat in hate_categories:
        ...
        for emo in unique_emotions:
            if cat_dummy.std() == 0 or df_clean[emo].std() == 0:
                r_val, p_val = np.nan, np.nan
            else:
                r_val, p_val = stats.pearsonr(cat_dummy, df_clean[emo])
            corr_row[emo] = r_val
            pval_row[emo] = p_val
    ...
    corrs_csv_path = os.path.join(results_dir, "correlation_pearson_hate_emotions.csv")
    pvals_csv_path = os.path.join(results_dir, "correlation_pvalue_hate_emotions.csv")
    
    df_corrs.to_csv(corrs_csv_path, float_format='%.4f')
    df_pvals.to_csv(pvals_csv_path, float_format='%.4f')
    
    print("=" * 100)
    print(" 2. COEFFICIENTS DE CORRELATION (Pearson r / Coefficient Phi) PAR PAIR HATE-EMOTION")
    print("=" * 100)
    ...
    chi2_results = []
    for emo in unique_emotions:
        contingency_table = pd.crosstab(df_clean[col_hate], df_clean[emo])
    ...
    df_chi2 = pd.DataFrame(chi2_results).set_index("Emotion")
    chi2_csv_path = os.path.join(results_dir, "association_chisquare_cramersv_hate_emotions.csv")
    df_chi2.to_csv(chi2_csv_path, float_format='%.4f')
    
    print("=" * 100)
    print(" 4. TEST DE CHI-DEUX ET V DE CRAMER (Association Globale HATE - Emotion)")
    print("=" * 100)
