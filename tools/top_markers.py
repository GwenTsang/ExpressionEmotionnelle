...
def print_top_markers_for_file(filepath: str, title: str, top_n: int = 20, export_path: str = None):
    ...
    if not prob_cols:
        print(f"ERREUR : Aucune colonne de probabilité P(...) trouvée dans {filepath}")
        return
...
def main():
    ...
    if os.path.exists(args.emotion):
        ...
        print(f"ATTENTION : Fichier d'émotions manquant : {args.emotion}")
    ...
    if os.path.exists(args.mode):
        ...
        print(f"ATTENTION : Fichier de modes manquant : {args.mode}")
